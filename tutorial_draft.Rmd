...
Now that we've generated the data, we can create the partial dependency using the
`partial.dep` arg:
...
Now that we have a partial dependency, we can isolate the partial dep of the `C`
hyperparameter:

```{r c_partial, message = FALSE, warning = FALSE}
...
```{r bivariate_heat, message = FALSE, warning = FALSE}
...
```{r bi_scatter, message = FALSE, warning = FALSE}
...
Provide an option so that we can see train/test on the same plot. This is harder to 
solve than it looks due to cases with nested CV and learner crashes. Naive implementation:
...
```{r feat_imp, message = FALSE, warning = FALSE}
